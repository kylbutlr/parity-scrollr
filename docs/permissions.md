...
| `scripting` | Installs scroll, navigation, and capture coordination in the two compared pages | After the user starts an approved comparison | Injection is limited to the active comparison tab; the script activates only for an approved direct child frame |
